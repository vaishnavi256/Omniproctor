...
class CustomWebEnginePage(QWebEnginePage):
    ...
    def createWindow(self, type):
        ...
        try:
            ...
            popup_view.setWindowTitle("Popup Window")
...
class SecureBrowser(QMainWindow):
    def __init__(self, url: str, python_exe_for_simplewall: str | None = None):
        ...
        # Start kiosk in background (avoid blocking UI)
        QTimer.singleShot(500, self.start_kiosk_protection_async)
        # Start network protection in background (avoid blocking UI)
        QTimer.singleShot(700, self.start_network_protection_async)
        # Check monitors after window is shown
        QTimer.singleShot(1200, self.check_monitors)
    ...
    def _on_network_ready(self):
        ...
        # small delay so UI can render spinner properly before navigation
        QTimer.singleShot(250, self.load_target_url)

    def _on_network_failed(self, reason: str):
        print("✗ Failed to activate network protection:", reason)
        # still proceed to load the URL
...
if __name__ == "__main__":
    ...
    url_args = [arg for arg in sys.argv[1:] if not arg.startswith('--') and arg.startswith('http')]
    if url_args:
